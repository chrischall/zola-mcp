...
describe('events & wedding tools', () => {
  ...
  it('listEvents: GETs event groups and flattens to event list', async () => {
    ...
    expect(reqSpy).toHaveBeenCalledWith('GET', '/v3/websites/events/wedding-accounts/1000001/groups');
  ...
  it('updateEvent: loads current event, merges fields, PUTs update', async () => {
    ...
    const result = await updateEvent({ event_id: 2000001, name: 'Updated Reception' });
